...
export default function PrecisionLock() {
  ...
  useEffect(() => {
    ...
    // Get all individual pieces (excluding the wireframe)
    const animatablePieces = Array.from(allPaths).filter(p =>
      p.getAttribute('fill') !== 'none' && p.getAttribute('opacity') !== '0'
    );

    // Set initial state - all pieces laying on the ground (bottom)
    animatablePieces.forEach((path, index) => {
      // Random horizontal spread along the bottom
      const offsetX = (Math.random() - 0.5) * 100; // -50 to 50 horizontal spread
      const offsetY = 120; // All pieces start at the bottom (ground)
      const rotation = (Math.random() - 0.5) * 90; // -45 to 45 degrees (laying down)
    ...
    // Animate each face sliding into position with micro-bounce
    animatablePieces.forEach((path, index) => {
      tl.to(path, {
        x: 0,
        y: 0,
        ...
        opacity: 1,
        duration: 0.7,
        ease: 'back.out(1.7)', // Creates micro-bounce effect
        onComplete: () => {
          // Brief scale pulse on lock (the "click" visual)
          gsap.to(path, {
            scale: 1.08,
            duration: 0.12,
            yoyo: true,
    ...
    // Final assembly confirmation - subtle green glow
    tl.to(svg, {
