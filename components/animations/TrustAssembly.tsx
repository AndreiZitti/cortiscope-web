...
export default function TrustAssembly() {
  ...
  useEffect(() => {
    ...
    // Separate paths by their fill color
    const outerHexagons = Array.from(allPaths).filter(p =>
      p.getAttribute('fill') === '#fff' || p.getAttribute('fill') === '#231F20'
    );
    const cubeFaces = Array.from(allPaths).filter(p =>
      p.getAttribute('fill') === '#00AEEF'
    );
    const wireframe = Array.from(allPaths).find(p =>
      p.getAttribute('stroke') === 'black' && p.getAttribute('fill') === 'none'
    );

    // Set initial state
    gsap.set([...outerHexagons, ...cubeFaces], {
      opacity: 0,
      scale: 0.8,
      transformOrigin: 'center'
    });
    gsap.set(wireframe, { opacity: 0 });

    const tl = gsap.timeline();

    // Build outer hexagons first
    outerHexagons.forEach((path, index) => {
      tl.to(path, {
        opacity: 1,
        scale: 1,
        duration: 0.4,
    ...
    // Then build cube faces piece by piece
    cubeFaces.forEach((path, index) => {
      tl.to(path, {
        opacity: 1,
        scale: 1,
        duration: 0.5,
        ...
        onStart: () => {
          gsap.to(path, {
            filter: 'drop-shadow(0 0 10px rgba(0, 174, 239, 0.9))',
            duration: 0.3,
            yoyo: true,
            repeat: 1,
          });
    ...
    // Light sweep across entire logo
    tl.to(svg, {
      filter: 'drop-shadow(0 0 25px rgba(0, 174, 239, 0.7))',
      duration: 0.7,
      ease: 'power2.inOut',
      yoyo: true,
      repeat: 1,
